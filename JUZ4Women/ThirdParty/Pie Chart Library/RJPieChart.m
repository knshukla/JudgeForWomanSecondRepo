...
@synthesize RJDataArray, RJColorsArray,RJPieChartView;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        self.pieRadius = 25;//MIN(frame.size.width/2, frame.size.height/2) - 10;
...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if(self)
    {
        ...
        [_pieView setBackgroundColor:[UIColor blackColor]];
...
//            [textLayer setBounds:CGRectMake(0, 0, size.width, size.height)];
            [textLayer setBounds:CGRectMake(0, 0, 10, 10)];
        }
    }
}
...
{
    if(!self.RJDataArray)
        RJDataArray = [[NSMutableArray alloc] init];
    if(!self.RJColorsArray)
        RJColorsArray = [[NSMutableArray alloc] init];
    
    self.RJDataArray = dataArray;
    self.RJPieChartView = layerHostingView;
    
    for(int i=0;i<self.RJDataArray.count;i++)
    {
        [self.RJColorsArray addObject:[UIColor colorWithRed:(rand()%255)/255.0 green:(rand()%255)/255.0 blue:(rand()%255)/255.0 alpha:1.0]];
    }
...
//    [layerHostingView setPieRadius:100];
    [layerHostingView setLabelFont:[UIFont fontWithName:@"DBLCDTempBlack" size:(24-RJDataArray.count/2)]];
...
//    [layerHostingView setPieCenter:CGPointMake(layerHostingView.pieRadius+OFFSET, layerHostingView.pieRadius+OFFSET)];
    ...
    [layerHostingView setCenter:self.center];
...
-(void)drawLegends:(RJPieChart *)layerHostingView dataArray:(NSMutableArray*)dataArray
{
    ...
    [legendsView setBackgroundColor:[UIColor blackColor]];
    ...
    for (int i=0;i<dataArray.count;i++)
    {
        ...
        btn.backgroundColor = [RJColorsArray objectAtIndex:i];
